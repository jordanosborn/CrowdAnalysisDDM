...
size_t get_stream_width(size_t stream_id)
{
    if (streams.size() > stream_id && streams[stream_id].isOpened())
    {
        return streams[stream_id].get(cv::CAP_PROP_FRAME_WIDTH);
    }
    else
    {
        return 0;
...
size_t get_stream_height(size_t stream_id)
{
    if (streams.size() > stream_id && streams[stream_id].isOpened())
    {
        return streams[stream_id].get(cv::CAP_PROP_FRAME_HEIGHT);
    }
    else
    {
        return 0;
