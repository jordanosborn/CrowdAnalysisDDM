...
\end{equation}
Taking the Taylor expansion of this equation and only retaining up to first order.
\begin{equation}
...
There are some notable caveats of this method which are outlined in detail in \cite{optical_flow}. The method therefore requires you to solve a partial differential equation thus necessitating implementation of some constraints in order to fully determine the motion. It also assumes that objects show consistent intensities which may or may not be valid depending on the footage. DDM requires neither of these things.
