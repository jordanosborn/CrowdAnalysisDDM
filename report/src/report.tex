...
\begin{subfigure}[t]{.5\textwidth}
  ...
  \caption{}
\end{subfigure}%
\caption{Here we see another example. The figures above show typical fits for the Brownian ISF at low (a), high (b) and mid-range (c) (d) \textit{q} for the video crowd5. The data was collected by running single-scale DDM (box-size = full frame). (A link to all data is available in section \ref{section:database})}
\label{fig:crowd4_brownian_fits}
\end{figure}
...
In this section we will show how the fitted parameters for "stationary" crowds vary with \textit{q} for a Brownian ISF fitted at a single-scale.
...
In this section we will show how the fitted parameters for "stationary" crowds vary with \textit{q} for a Ballistic ISF fitted at a single-scale.
...
\begin{subfigure}[t]{.5\textwidth}
  ...
  \caption{}
\end{subfigure}%
\caption{Here we see another example. The figures above show typical fits for the Brownian ISF at low (a), high (b) and mid-range (c) (d) \textit{q} for the video running6. The data was collected by running single-scale DDM (box-size = full frame). The fits shown are exceptionally poor and the data itself is particularly noisy. This would suggest that the motion in this crowd is not of a Brownian nature and therefore the crowd is incorrectly classified as belonging to the Swimmer like group. (A link to all data is available in section \ref{section:database})}
...
In this section we will show how the fitted parameters for swimmer-like crowds vary with \textit{q} for a Brownian ISF fitted at a single-scale.
...
In this section we will show how the fitted parameters for swimmer-like crowds vary with \textit{q} for a Ballistic ISF fitted at a single-scale.
...
\paragraph{Brownian ISF} In this section we will show how the parameters from swimmer-like crowds fitted to a Brownian ISF vary as a function of \textit{q} at different box-sizes. 
\begin{figure}[H]
\begin{subfigure}[t]{\textwidth}
  \centering
...
\paragraph{Ballistic ISF} In this section we will show how the parameters from swimmer-like crowds fitted to a Ballistic ISF vary as a function of \textit{q} at different box-sizes. 
