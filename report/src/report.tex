...
\subsection{Database}
The raw data from DDM is stored inside an Sqlite database which can be found at \url{todo.com}. Naming conventions are used such that tables are named according to the format $\{$video name$\}\_\{$data type$\}$ replacing the curly braces with the name of the video and data type that are desired.
