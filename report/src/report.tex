...
\begin{enumerate}
\item It can be applied without any special preparations of the system. Can be used on pre-recorded footage.
\item Does not require training data.
\item Is completely autonomous (once an ISF) is selected.
\item Is highly parallelisable so could be carried out in real-time.
\item It can be applied to a wide range of dynamical systems.
\item Is based on the physical information encoded in the footage, rather than focusing on particular image properties.
\item It can be implemented inexpensively, only requirements are some video footage (pre-recorded, or from a camera) and a mid-range computer.
\end{enumerate}
